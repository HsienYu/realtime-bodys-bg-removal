...
# Background frame offset for original background modes
background_frame_offset = 5
...
def get_background_frame(original_frame):
    ...
    if (bg_mode == 3 or bg_mode == 9) and background_image is not None:
        ...
        # Original video background - use a frame from adjustable offset ago
        if len(original_background_buffer) > background_frame_offset:
            # Use frame from background_frame_offset frames ago
            return original_background_buffer[-background_frame_offset]
        else:
            ...
            try:
                if is_transparent:
                    success = publish_frame_to_ndi(
                        frame_to_publish, ndi_sender, is_rgba=True, fps=target_ndi_fps)
                else:
                    success = publish_frame_to_ndi(
                        frame_to_publish, ndi_sender, fps=target_ndi_fps)
                ...
                if ndi_stats['sent'] % 100 == 0 and ndi_stats['sent'] > 0:
                    ...
                    print(
                        f"NDI: {ndi_stats['sent']} sent, {success_rate:.1f}% success, queue_full: {ndi_stats['queue_full']}")
...
def handle_keys(key):
    ...
    if key == ord('q'):
        ...
        if bg_mode in [4, 10]:
            ...
            print(
                f"Background frame offset increased to: {background_frame_offset} frames ago")
        else:
            print(
        ...
        if bg_mode in [4, 10]:
            ...
            print(
                f"Background frame offset decreased to: {background_frame_offset} frames ago")
        else:
            print(
                "Background frame offset only works in Original Background modes (4, 10)")
...
print("  3 - Keep Body - Transparent BG")
print("  4 - Keep Body - Original BG")
print("  5 - Remove Body - Green Fill")
print("  6 - Remove Body - Blue Fill")
print("  7 - Remove Body - Transparent Area")
print("  8 - Remove Body - Original BG")
