...
original_background_buffer = deque(maxlen=30)  # Store recent frames for background
...
def get_background_frame(original_frame):
    ...
    if (bg_mode == 3 or bg_mode == 9) and background_image is not None:
        ...
        if len(original_background_buffer) > 5:
            return original_background_buffer[-5]  # Use frame from 5 frames ago
        else:
...
def handle_keys(key):
    ...
    if key == ord('q'):
        ...
        print(f"Processing frequency increased: Every {process_every_n_frames} frame(s)")
    elif key == ord('-'):
        process_every_n_frames += 1
        print(f"Processing frequency decreased: Every {process_every_n_frames} frame(s)")
...
while is_running:
    ...
    if bg_mode in [2, 8] and display_frame is not None:
        ...
        checker_pattern = np.zeros((checker_size*2, checker_size*2, 3), dtype=np.uint8)
